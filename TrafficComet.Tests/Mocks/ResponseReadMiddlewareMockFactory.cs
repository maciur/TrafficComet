...
{
	internal static class ResponseReadMiddlewareMockFactory
	{
		internal static ResponseReadMiddleware CreateMockObject(out HttpContext httpContext,
			ResponseReadMiddlewareConfig config = null, string requestPath = null)
		{
			...
			var middlewareConfig = config ?? new ResponseReadMiddlewareConfig();

			return new ResponseReadMiddleware(MockServicesHelper.FakeRequestDelegate,
